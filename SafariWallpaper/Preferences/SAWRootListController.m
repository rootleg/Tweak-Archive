...
- (void)viewDidLoad {
    ...
    self.appearanceSettings = [SAWAppearanceSettings new];
    self.hb_appearanceSettings = [self appearanceSettings];


    self.preferences = [[HBPreferences alloc] initWithIdentifier:@"love.litten.safariwallpaperpreferences"];


    self.enableSwitch = [UISwitch new];
    [[self enableSwitch] setOnTintColor:[UIColor colorWithRed:0.95 green:0.63 blue:0.45 alpha:1]];
    ...
    [[self iconView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/SafariWallpaperPreferences.bundle/icon.png"]];
    ...
    [[self headerImageView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/SafariWallpaperPreferences.bundle/Banner.png"]];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/Safariwallpaper.disabled"]) {
        [[self enableSwitch] setEnabled:NO];

        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Safariwallpaper" message:@"Safariwallpaper has detected that you have disabled it with iCleaner Pro, here are some quick actions you can perform" preferredStyle:UIAlertControllerStyleAlert];
        ...
        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Safariwallpaper" message:@"Safariwallpaper has detected that you have DayNightSwitch installed, which causes issues with Safariwallpaper's preferences\n\n To continue, please disable DayNightSwitch with iCleaner Pro or uninstall it temporarily" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetPreferences {
    ...
    [[NSFileManager defaultManager] removeItemAtPath:@"/var/mobile/Library/Preferences/love.litten.safariwallpaperpreferences/" error:nil];
...
- (void)respring {
    ...
    [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib"])
            ...
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Tweaks&path=Safariwallpaper"]];
    }];
