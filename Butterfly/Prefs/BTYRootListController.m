...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/Butterfly.disabled"]) return;
    [[self enableSwitch] setEnabled:NO];

    UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Butterfly"
	message:@"Butterfly Preferences disabled due to Butterfly being disabled with iCleaner Pro"
...
- (void)toggleState {
    ...
    NSString* path = [NSString stringWithFormat:@"/var/mobile/Library/Preferences/love.litten.butterflypreferences.plist"];
    NSMutableDictionary* dictionary = [NSMutableDictionary dictionaryWithContentsOfFile:path];
    NSSet* allKeys = [NSSet setWithArray:[dictionary allKeys]];
    HBPreferences *preferences = [[HBPreferences alloc] initWithIdentifier: @"love.litten.butterflypreferences"];
    
    if (!([[NSFileManager defaultManager] fileExistsAtPath:@"/var/mobile/Library/Preferences/love.litten.butterflypreferences.plist"])) {
        enabled = YES;
