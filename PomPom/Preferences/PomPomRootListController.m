//
//  PomPomRootListController.m
//  PomPom
//
//  Created by Alexandra (@Traurige)
//

#include "PomPomRootListController.h"

@implementation PomPomRootListController
- (void)viewDidLoad {
    [super viewDidLoad];

    self.appearanceSettings = [PomPomAppearanceSettings new];
    self.hb_appearanceSettings = [self appearanceSettings];

    self.preferences = [[HBPreferences alloc] initWithIdentifier:@"auraoflove.pompompreferences"];
    ...
    [[self iconView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/PomPomPreferences.bundle/Icon.png"]];
    ...
    [[self headerImageView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/PomPomPreferences.bundle/Banner.png"]];
    ...
    // if pompom was disabled through icleaner pro show an alert with options to reset the preferences, go back or ignore it
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/PomPom.disabled"]) {
        [[self enableSwitch] setEnabled:NO];

        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"PomPom" message:@"PomPom has detected that you have disabled it with iCleaner Pro, here are some quick actions you can perform" preferredStyle:UIAlertControllerStyleAlert];
    ...
    // tell the user that the tweak daynightswitch breaks the enable switch in the top right corner
        ...
        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"PomPom" message:@"PomPom has detected that you have DayNightSwitch installed, which causes issues with PomPom's preferences\n\n To continue, please disable DayNightSwitch with iCleaner Pro or uninstall it temporarily" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetPrompt {
    UIAlertController* resetAlert = [UIAlertController alertControllerWithTitle:@"PomPom" message:@"Do you really want to reset your preferences?" preferredStyle:UIAlertControllerStyleActionSheet];
...
- (void)respring {
    ...
    [UIView animateWithDuration:1 delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib"])
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=PomPom"]];
        else
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Tweaks&path=PomPom"]];
    }];
}
@end
